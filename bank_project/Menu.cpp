...
{
	char temp[150];//this will be better 
	strcpy_s(temp, sizeof temp, "SELECT stan FROM bank.all_data WHERE login = ");//adding chars using strcpy_s() function
	strcat_s(temp, sizeof temp, "'");
	strcat_s(temp, sizeof temp, actual_acc.Username.c_str());
	strcat_s(temp, sizeof temp, "'");
	static std::string temp1 = temp;// this is must static because 
...
{
	char temp[150];//this will be better 
	strcpy_s(temp, sizeof temp, "SELECT data FROM bank.all_data WHERE login = ");//adding chars using strcpy_s() function
	strcat_s(temp, sizeof temp, "'");
	strcat_s(temp, sizeof temp, actual_acc.Username.c_str());
	strcat_s(temp, sizeof temp, "'");
	static std::string temp1 = temp;// this is must static because 
